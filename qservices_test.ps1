...
## Thisloop will only TEST the current state of all selected Services ##
$TEST = {$Diagnostic = Get-Service -ComputerName "$Server" | Where-Object {$_.name -eq "$Service"}
    Start-Sleep -s $Wait
    }

## Start logging ##
Start-Transcript -Path $Admin\$DesktopFolder\$ScriptFolder\$LogFolder\$Server-$Date-test.log -Append
...
## Running State ##
$State = $TEST
Write-Host "`n2. TEST mode selected, this is the current state of selected Services:"
